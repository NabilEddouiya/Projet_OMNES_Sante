...
<?php
    ...
    if ($db_found) { 
        ...
        $sql1 = "SELECT DISTINCT c.ID_Consultation,m.Nom_m,cl.Nom,c.date_heure_debut,c.WeekDay,c.Creneau FROM consultation c, client cl, medecin m WHERE (cl.Email=c.EmailClient AND cl.Email='$email' AND c.Medecin_ID=m.ID)";
        $resultat = mysqli_query($db_handle, $sql1);
        if(mysqli_num_rows($resultat)!=0) {
            ...
            while($data = mysqli_fetch_assoc($resultat)) {
                ...
                $heuredebut = $data["WeekDay"];
                $heurefin = $data["Creneau"];
            }
            echo "</table> <br><br>";
        }
        else {
